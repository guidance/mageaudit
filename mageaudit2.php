...
{
    $modulesDir = BP . DIRECTORY_SEPARATOR .
        'app' . DIRECTORY_SEPARATOR .
        'code' . DIRECTORY_SEPARATOR ;
    $directory = new RecursiveDirectoryIterator($modulesDir);
    $iterator = new RecursiveIteratorIterator($directory);
    $regexIterator = new RegexIterator($iterator, '~^.*registration\\.php$~i', RecursiveRegexIterator::GET_MATCH);
    ...
    foreach ($regexIterator as $filename) {
        ...
        $arguments = array();
        if (preg_match_all(
                '/\\:\\:\\s*register\\s*\\(\\s*([^\\)]*)\\s*\\)\\s*;/is',
                $phpContent,
                $arguments
            )
        ) {
            foreach($arguments[1] as $argContent) {
                $argContent = trim($argContent);
                if ($argContent) {
                    $moduleNames = array();
                    if (preg_match('/[^,]*module[^,]*,\\s*(?:\'|")([^,]*)(?:\'|")\\s*,.*/is', $argContent, $moduleNames)) {
                        $moduleName = trim($moduleNames[1]);
                        if ($moduleName) {
                            $modules[] = array('name' => $moduleName, 'depends' => array() /* not implemented yet */);
                        }
                    }
                }
            }
    ...
    if ($sorted) {
        usort($modules, 'sortModules');
    }
    return array(array('local' => $modules), array()/* should be dependencies */);
}
